...
auto HttpSession::should_use_write_some() -> bool {
  return http_response.body().size() > READ_SOME_THRESHOLD ||
         http_response[boost::beast::http::field::content_type] ==
             "application/octet-stream" ||
         http_response[boost::beast::http::field::content_type] == "video/mp4";
}

auto HttpSession::write_response() -> void {
  auto self = shared_from_this();
  http::async_write(tcp_socket, http_response,
...
auto HttpSession::write_response_some() -> void {
  ...
  tcp_socket.async_write_some(
      boost::asio::buffer(http_response.body().data(),
